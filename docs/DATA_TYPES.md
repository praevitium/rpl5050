...
**Last updated.** Session 258 (2026-04-26, BinaryInteger B-column rejection-pin pass;
...
| %CH | ✓ | ✓ | ✓ | ✗ | ✗ | ✓ | ✓  | ✓ | ✗ | ✗ | ✓ | · | ✗ | ✗ | Same. Session 120 pinned both-side Tagged tag-drop. Session 231 pinned Q=✓: `Rational(1,2) Real(1) %CH` → `Real(100)` (Q→R degradation). **Session 258:** B `·`→`✗` — same `toRealOrThrow` path as %. |
