...
| %T  | ✓ | ✓ | ✓ | · | ✗ | ✓ | ✓  | ✓ | ✗ | ✗ | ✓ | · | ✗ | ✗ | Same. Infinite result on base = 0 preserved. Session 120 pinned both-side Tagged tag-drop. Session 130 pinned right-side Tagged-of-List composition: `50 :p:{25 75} %T` → `{Real(50), Real(150)}` (right-Tagged + scalar × List broadcast). Session 231 pinned Q=✓: `Rational(1,4) Real(1) %T` → `Real(400)` (Q→R degradation). |
| %CH | ✓ | ✓ | ✓ | · | ✗ | ✓ | ✓  | ✓ | ✗ | ✗ | ✓ | · | ✗ | ✗ | Same. Session 120 pinned both-side Tagged tag-drop. Session 231 pinned Q=✓: `Rational(1,2) Real(1) %CH` → `Real(100)` (Q→R degradation). |
